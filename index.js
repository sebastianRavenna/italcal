...
function initializeFixedNavigation() {
    ...
    function handleFixedNav() {
        ...
        // Calculamos cuánto del header ya se ha desplazado
        const headerScrolled = Math.min(scrollTop, headerHeight);
        
        if (scrollTop >= headerHeight) {
            // El navbar debe volverse fijo cuando el header desaparece completamente
            navbar.classList.add('fixed-nav');
            document.body.classList.add('nav-fixed');
        } else {
            // El navbar debe seguir al header mientras este se desplaza
            navbar.classList.remove('fixed-nav');
            document.body.classList.remove('nav-fixed');
            
            // Aplicar transform al navbar para que "suba" con el scroll
